...
import hu.bme.mit.sysml.oosem.generators.GeneratorUtils

class SpecificationToDesignGenerator implements IGenerator {
	override String generate(BlockGenerationData data) {
		return '''
			package «data.blockName» {
			    ...
			    #design «GeneratorUtils.getSysMLType((data.subject.object as Type))» def «data.blockName» :> «((data.subject.object as Type)).name» {
